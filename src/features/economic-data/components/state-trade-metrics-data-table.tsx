...
import { stateTradeMetricColumns } from "./state-trade-metrics-table-columns";

const stateNameOrCodeGlobalFilter: FilterFn<StateTradeMetricRow> = (
  row,
  _columnId,
  filterValue,
) => {
...
type StateTradeMetricsDataTableProps = {
  "aria-label"?: string;
  data: StateTradeMetricRow[];
  globalFilter: string;
  onGlobalFilterChange: (value: string) => void;
};

export function StateTradeMetricsDataTable({
  "aria-label": ariaLabel = "State trade metrics by state and year",
  data,
  globalFilter,
  onGlobalFilterChange,
}: StateTradeMetricsDataTableProps) {
  ...
  const table = useReactTable({
    data,
    columns: stateTradeMetricColumns,
    getRowId: (row) => row.id,
  ...
  return (
    <div className="-mx-px touch-pan-x overflow-x-auto rounded-md border">
      <table aria-label={ariaLabel} className="w-full min-w-[36rem] text-left text-sm sm:min-w-[40rem]">
          ...
          {noMatches ? (
            <tr>
              <td
                ...
                colSpan={stateTradeMetricColumns.length}
              >
