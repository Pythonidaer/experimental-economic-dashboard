...
import { stateIndustriesColumns } from "./state-industries-table-columns";

const industryGlobalFilter: FilterFn<StateIndustryRow> = (row, _columnId, filterValue) => {
  const q = String(filterValue ?? "")
    .toLowerCase()
    .trim();
  if (!q) return true;

  const { region, industry, naics_code } = row.original;
  return (
...
type StateIndustriesDataTableProps = {
  "aria-label"?: string;
  "aria-describedby"?: string;
  data: StateIndustryRow[];
  globalFilter: string;
  onGlobalFilterChange: (value: string) => void;
};

export function StateIndustriesDataTable({
  "aria-label": ariaLabel = "Industry employment comparison by region",
  "aria-describedby": ariaDescribedBy,
  data,
  globalFilter,
  onGlobalFilterChange,
}: StateIndustriesDataTableProps) {
  ...
  const table = useReactTable({
    data,
    columns: stateIndustriesColumns,
    getRowId: (row) => row.id,
    ...
    globalFilterFn: industryGlobalFilter,
  });
          ...
          {noMatches ? (
            <tr>
              <td
                ...
                colSpan={stateIndustriesColumns.length}
              >
                No rows match your filter. Try a different region, industry, or NAICS code.
              </td>
            </tr>
          ) : (
            rows.map((row) => (
